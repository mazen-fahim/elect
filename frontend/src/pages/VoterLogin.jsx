  ...
  let handleRequestOtp = async (e) => {
    ...
    try {
      ...
      if (election.method === "api" && election.api_endpoint) {
        ...
        console.log(
          "Step 2: API-based election - checking voter eligibility with dummy service...",
        );

        const dummyServiceUrl =
          "http://localhost/api/proxy/dummy-service/verify-voter/public";
        ...
        if (!dummyServiceResponse.ok) {
          throw new Error(
            `Dummy service error: ${dummyServiceResponse.status}`,
          );
        }

        const dummyServiceData = await dummyServiceResponse.json();
        console.log(
          "Step 2 COMPLETED: Dummy service response:",
          dummyServiceData,
        );

        if (!dummyServiceData.is_eligible) {
          throw new Error(
            dummyServiceData.error_message ||
              "Voter not eligible for this election",
          );
        }

        console.log(
          "Step 3: Voter verified by dummy service, requesting OTP from backend...",
        );
        ...
        console.log(
          "Step 2: CSV-based election - directly requesting OTP from backend...",
        );
          ...
          <form onSubmit={handleRequestOtp} className="space-y-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Enter your national ID to receive an OTP on your registered
                phone number
              </p>
            </div>
