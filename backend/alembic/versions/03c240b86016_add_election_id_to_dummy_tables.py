...
down_revision: Union[str, Sequence[str], None] = '59efc9b6b402'
...
def upgrade() -> None:
    ...
    op.create_foreign_key('fk_dummy_candidates_election_id', 'dummy_candidates', 'elections', ['election_id'], ['id'], ondelete='CASCADE')
    
    # Add election_id to dummy_voters table
    op.add_column('dummy_voters', sa.Column('election_id', sa.Integer(), nullable=False))
    op.create_foreign_key('fk_dummy_voters_election_id', 'dummy_voters', 'elections', ['election_id'], ['id'], ondelete='CASCADE')
