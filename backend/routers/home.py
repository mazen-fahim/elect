...
async def get_public_elections(
    ...
    if country:
        filters.append(Organization.country == country)
    ...
    for election_data in elections_data:
        ...
        elections.append(PublicElection(
            id=election_data.id,
            ...
            organization_country=election_data.organization_country,
    ...
    return PublicElectionsResponse(
        elections=elections,
        total=total,
        page=page,
        limit=limit,
...
async def get_filter_options(db: db_dependency):
    ...
    orgs_result = await db.execute(
        select(Organization.name).distinct().where(Organization.status == "accepted")
    )
